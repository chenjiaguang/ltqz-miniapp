// components/shoppingView/shoppingView3.js
...
Component({
  ...
  methods: {
    formatData: function (data) {
      let ticket_group = []
      data.forEach((item, idx) => { // 票的价格处理(String -> Number，分保留，另存一份用于展示的价格, 计算时用分，展示时
        item.show_price =  util.formatMoney(item.price).showMoney
        item.price =  util.formatMoney(item.price).money
        item.show_pt_price =  util.formatMoney(item.pt_price).showMoney
        item.pt_price =  util.formatMoney(item.pt_price).money
        ...
        item.qg_price =  util.formatMoney(item.qg_price).money
        if (!item.relation) {
          item.relation = {lv2: item.sub ? item.sub.map(item => item.id) : []}
        }
        if (!ticket_group[0]) {
          ticket_group.push({title: item.type, id: 'lv1', list: []})
        }
        ticket_group[0].list.push(item)
        if (!ticket_group[1]) {
          if (item.sub && item.sub[0]) {
            ticket_group.push({title: item.sub[0].type, id: 'lv2', list: []})
          }
        }
        ...
        if (item.sub && item.sub[0]) {
          item.sub.forEach(sitem => {
            ...
            sitem.qg_price =  util.formatMoney(sitem.qg_price).money
            if (!sitem.relation) {
              sitem.relation = {lv1: [item.id], lv3: sitem.sub ? sitem.sub.map(ssitem => ssitem.id) : []}
            }
            ticket_group[1].list.push(sitem)
          })
        }
      })
      ticket_group.forEach(item => {
        let _list = []
        let _list_id = []
        item.list.forEach(litem => {
          if (_list_id.indexOf(litem.id) === -1) {
            _list.push(litem)
            _list_id.push(litem.id)
          }
        })
        item.list = _list
      })
      this.setData({ticket_group})
    },
    initStyle: function (saletype, callback) {
      const {ticket_group, stockObj} = this.data
      if (!ticket_group || !ticket_group.length) { // 未初始化过数据前
        return false
      }
      let _obj = {
        saletype
      }
      ticket_group.forEach((item, idx) => {
        item.list.forEach((litem, lidx) => {
          if (litem[stockObj[saletype]] === 0) {
            _obj[`ticket_group[${idx}].list[${lidx}].disabled`] = true
          }
        })
      })
      console.log('_obj', _obj, ticket_group)
      this.setData(_obj, callback)
    },
    styleTap: function (e) {
      const {id, level} = e.currentTarget.dataset
      const {ticket_group, stockObj, saletype} = this.data
      let countStyle = null
      let list = ticket_group.filter(item => item.id === level)[0].list
      let idx = null
      list.forEach((litem, lidx) => {
        if (litem.id === id) {
          idx = lidx
        }
      })
      let target = list[idx]
      if (target[stockObj[saletype]] === 0 || target[stockObj[saletype]] === '0') { // 没有库存
        return false
      }
      ...
      ticket_group.forEach(titem => {
        if (titem.id === level && target.selected) { // 同级，并且当前项为选中状态
          titem.list.forEach(litem => {
            if (litem.id !== id) {
              litem.selected = false
            }
          })
        } else if (titem.id !== level) {
          titem.list.forEach(litem => {
            if (litem[stockObj[saletype]] === 0 || litem[stockObj[saletype]] === '0') {
              litem.disabled = true
            } else if (litem.relation) {
              for (let key in litem.relation) {
                console.log('ticket_group.filter(ttitem => ttitem.id === key)[0].list', ticket_group.filter(ttitem => ttitem.id === key)[0].list, litem.relation, key, litem.relation[key])
                if (ticket_group.filter(ttitem => ttitem.id === key)[0].list.filter(sitem => sitem.id === litem.relation[key])[0].selected) {
                  litem.disabled = false
                }
              }
            }
          })
      ...
      // if (target.selected) { // 选中时
      //   list.forEach(item => { // 同一级，非当前项置为未选中(单选)
      //     if (item.id !== id) {
      //       item.selected = false
      //     }
      //   })
      ...
      // if (target.relation) { // 非同级存在时
      //   for (let key in target.relation) {
      //     target.relation[key].forEach(item => {
      //       if (ticket_group.filter(titem => titem.id === key)[0].list.filter(sitem => sitem.id === item)[0].selected) {
      //         item.disabled = false
      //       }
      //       if (item[stockObj[saletype]] === 0 || item[stockObj[saletype]] === '0') {
      //         item.disabled = true
      //       }
      //     })
      //   }
      // }
      let allStyleHasSelected = true
      for (let i = 0; i < ticket_group.length; i++) {
        if (!ticket_group[i].list.filter(item => item.selected)[0]) {
          allStyleHasSelected = false
          break
        }
      }
      if (allStyleHasSelected) { // 每个规格都有选中的款式
        countStyle = ticket_group[ticket_group.length - 1].list.filter(item => item.selected)[0]
        countStyle.num = 0
      }
      this.setData({
        ticket_group,
      ...
      console.log('target', ticket_group, countStyle)
    },
    ...
    countTicket: function (e) {
      ...
      const singlePrice = (subSessions && subSessions.length > 0 && (currentSubSession[saletype] || currentSubSession[saletype] === 0)) ? subSessions[currentSubSession[saletype]][singlePriceObj[saletype]] : session[singlePriceObj[saletype]]
      total = num * singlePrice
      console.log('singlePrice', singlePrice, saletype, (subSessions && subSessions.length > 0 && currentSubSession[saletype]), subSessions[currentSubSession[saletype]][singlePriceObj[saletype]])
      let _obj = {}
    ...
    toggleSession: function (e) {
      ...
      if (_saletype != saletype) { // 点击的购买方式不同时，清空原选择数据
        this.initStyle(_saletype, this.toggleModal)
      } else {
        this.toggleModal()
      }
    },
